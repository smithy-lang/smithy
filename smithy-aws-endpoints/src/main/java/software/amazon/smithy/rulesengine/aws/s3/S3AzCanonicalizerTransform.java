...
final class S3AzCanonicalizerTransform extends TreeMapper {
    ...
    private boolean isAzIdSubstringBinding(Condition cond) {
        if (!cond.getResult().isPresent() || !ID_AZ_ID.equals(cond.getResult().get())) {
            return false;
        }
    ...
    private Condition createCanonicalAzCondition(Condition original) {

...
public final class StandardOperationVerbValidator extends AbstractValidator {
    ...
    public static ValidatorService provider() {
        return ValidatorService.createProvider(StandardOperationVerbValidator.class, node -> {
            var verbs = Node.loadArrayOfString(
            ...
            if (verbs.isEmpty() && suggestAlternatives.isEmpty()) {
                throw new SourceException(
                        "Either verbs or suggestAlternatives must be set when configuring StandardOperationVerb", node);
            }

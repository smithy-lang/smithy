...
public final class NoninclusiveTermsValidator extends AbstractValidator {
        ...
        if (!config.getExcludeDefaults()) {
            ...
            if (config.getTerms().isEmpty()) {
                ...
                throw new IllegalArgumentException(
                    "Cannot set 'excludeDefaults' to true and leave "
        ...
        switch (instance.getLocationType()) {
            case NAMESPACE:
                ...
                return ValidationEvent.builder()
                    ...
                    .id(
                        getName() + "." + NAMESPACE + "." + instance.getText()
                            + "." + matchedText.toLowerCase(Locale.US)
                    )
                    .message(
                        String.format(
                            "%s namespace uses a non-inclusive term `%s`.%s",
                            instance.getText(),
                            matchedText,
                            replacementAddendum
                        )
            ...
            case APPLIED_TRAIT:
                ...
                if (instance.getTraitPropertyPath().isEmpty()) {
                    return validationEvent.toBuilder()
                        .message(
                            String.format(
                                "'%s' trait has a value that contains a non-inclusive term `%s`.%s",
                                idiomaticTraitName,
                                matchedText,
                                replacementAddendum
                            )
                    ...
                    return validationEvent.toBuilder()
                        ...
                        .id(
                            getName() + "." + TRAIT + "." + matchedText.toLowerCase(Locale.US)
            ...
            default:
                return warning(
                    ...
                    String.format(
                        "%s shape uses a non-inclusive term `%s`.%s",
    ...
    private static String getReplacementAddendum(String matchedText, List<String> replacements) {
        List<String> caseCorrectedEntryValue = replacements.stream()
            .map(
                replacement -> Character.isUpperCase(matchedText.charAt(0))
        ...
        String replacementAddendum = !replacements.isEmpty()
            ? String.format(
                " Consider using one of the following terms instead: %s",

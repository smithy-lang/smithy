...
description: >-
  ...
  index and the reStructuredText (.rst) source of each page - the complete,
  version-matched text a summary would drop. Trigger phrases include "in
  Smithy", "Smithy model", "smithy.io", "what does the Smithy spec say", "how
  do I define ... in Smithy", "is this a breaking change", "which Smithy
  codegen".
license: Apache-2.0
---
...
3. **Look up a specific trait through the trait index** instead of reading a
   ...
   for example `spec/constraint-traits.html#length-trait`. To answer a
   trait-specific question:

   1. Read the trait index and find the trait's link. The anchor (the part
      after `#`, e.g. `length-trait`) names the section.
   2. Rewrite the linked page to its `.rst` source with the rule in step 2
      (e.g. `spec/constraint-traits.html` becomes
      `.../docs/source-2.0/spec/constraint-traits.rst`).
   3. Read that `.rst` source and jump to the matching label line
      (`.. _length-trait:`), then read from there to the next label or heading. That
      is the authoritative definition of just that one trait, without loading
      every trait.

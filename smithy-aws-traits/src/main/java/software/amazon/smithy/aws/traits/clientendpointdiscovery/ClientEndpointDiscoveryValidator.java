...
public final class ClientEndpointDiscoveryValidator extends AbstractValidator {
    ...
    public List<ValidationEvent> validate(Model model) {
        ...
        validationEvents.addAll(
            endpointDiscoveryServices.values()
                .stream()
                ...
                .map(Optional::get)
                .flatMap(
                    endpointOperation -> validateEndpointOperation(
                        model,
                        opIndex,
                        endpointOperation
                    ).stream()
    ...
    private List<ValidationEvent> validateServices(
        ...
        validationEvents.addAll(
            endpointDiscoveryServices.keySet()
                .stream()
    ...
    private List<ValidationEvent> validateOperation(OperationShape operation, List<ClientEndpointDiscoveryInfo> infos) {
        ...
        return infos.stream()
            ...
            .filter(
                discoveryInfo -> !operation.getErrors()
                    .contains(

...
public final class ClientEndpointDiscoveryValidator extends AbstractValidator {
    ...
    public List<ValidationEvent> validate(Model model) {
        ...
        validationEvents.addAll(endpointDiscoveryServices.values()
                .stream()
    ...
    private List<ValidationEvent> validateTrait(ServiceShape service, ClientEndpointDiscoveryTrait trait) {
        if (!trait.getOptionalError().isPresent()) {
            return ListUtils.of(danger(service,
                    trait,
    ...
    private List<ValidationEvent> validateServices(
        ...
        List<ValidationEvent> validationEvents = endpointDiscoveryServices.entrySet()
                ...
                .map(pair -> error(pair.getKey(),
                        String.format(
                                "The operation `%s` must be bound to the service `%s` to use it as the endpoint operation.",
                                pair.getValue().getOperation(),
                                pair.getKey()),
                        UNBOUND_OPERATION,
                        pair.getValue().getOperation().getName()))
                .collect(Collectors.toList());

        validationEvents.addAll(endpointDiscoveryServices.keySet()
                .stream()
                .filter(service -> discoveryIndex.getEndpointDiscoveryOperations(service).isEmpty())
                .map(service -> warning(service,
                        String.format(
        ...
        Optional.ofNullable(outputMembers.get("Endpoints"))
                ...
                .ifPresent(pair -> {
                    ...
                    if (!listShape.isPresent()) {
                        events.add(error(pair.getLeft(),
                                "The output member `Endpoints` on an endpoint discovery "
                    ...
                    if (!listMember.isPresent()) {
                        events.add(error(listShape.get(),
                                "The member of the Endpoints list in an "
                    ...
                    if (address.isPresent() && !address.get().isStringShape()) {
                        events.add(error(addressMember.get(),
                                "The `Address` member of the `Endpoint` shape must "
                                        + "be a string type."));
                    }
                    ...
                    if (cachePeriod.isPresent() && !cachePeriod.get().isLongShape()) {
                        events.add(error(cachePeriodMember.get(),
                                "The `CachePeriodInMinutes` member of the "
                    ...
                    if (!memberNames.equals(SetUtils.of("Address", "CachePeriodInMinutes"))) {
                        events.add(error(listMember.get(),
                                String.format(
    ...
    private List<ValidationEvent> validateEndpointOperationInput(
        ...
        input.getMember("Operation")
                ...
                .ifPresent(shape -> events.add(error(
                        shape,
                        "The Operation member of an endpoint discovery operation must be a string")));
        ...
        input.getMember("Identifiers")
                ...
                .ifPresent(pair -> {
                    ...
                    events.add(error(pair.getLeft(),
                            "The Identifiers member of an endpoint discovery "

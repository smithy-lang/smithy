...
The implementation lives in `smithy-model` under the package
`software.amazon.smithy.model.loader.smf`.

### Package Structure

```
smithy-model/src/main/java/software/amazon/smithy/model/loader/smf/
├── SmfWriter.java            // Serializes a Model to SMF bytes
├── SmfReader.java            // Deserializes SMF bytes to a Model
├── SmfSharedSymbols.java     // Shared symbol table constants (version 1)
├── SmfConstants.java         // Magic, format version, shape type enums, value tags
├── LEB128.java                // VarUInt/VarInt encoding and decoding
└── SmfModelLoader.java       // Integration with ModelLoader (file extension dispatch)
```

### Phases

**Phase 1: Core primitives and constants**

- `SmfConstants`: magic number, format version, shape type enum, dynamic
  value tags, flag bits.
- `SmfSharedSymbols`: the shared symbol table (version 1) as a `String[]`
  constant and a reverse lookup `Map<String, Integer>`.
- `LEB128`: static methods for `readVarUInt(byte[], int)`,
  `writeVarUInt(byte[], int, int)`, `readVarInt(byte[], int)`,
  `writeVarInt(byte[], int, long)`. Returns new position to avoid object
  allocation.

**Phase 2: Writer**

- `SmfWriter.write(Model): byte[]`
- Builds the local symbol table by iterating all shapes, collecting all
  unique strings (shape IDs, trait names, member names), subtracting shared
  symbols, and sorting by frequency.
- Writes header, symbol table, shape index (placeholder offsets, then
  backpatched), metadata, and shapes.
- Handles all shape types and dynamic value encoding.
- Supports a `stripped` option to omit `smithy.api#documentation` traits.

**Phase 3: Reader**

- `SmfReader.read(byte[]): Model`
- Reads header, builds symbol table array, reads shape index, reads metadata,
  reads shapes.
- Constructs shapes directly using builders (same builders as
  `AstModelLoader`).
- Trait values are constructed as `Node` objects and passed to `TraitFactory`.
- Integrates with `ModelAssembler` via `SmfModelLoader` which registers the
  `.smf` extension.

**Phase 4: ModelLoader integration**

- `SmfModelLoader` registered in `ModelLoader.load()` for `.smf` files.
- JAR discovery updated to recognize `.smf` files in manifests.
- `ModelSerializer` gains a companion `SmfWriter` option (or a separate
  entry point).

**Phase 5: Selective loading**

- `SmfReader.readSelective(byte[], Set<ShapeId>): Model`, reads only the
  shapes in the provided set plus their transitive closure.
- Closure computation uses the shape index for offset lookup and parses
  shapes on demand.
...
Tests live in:
```
smithy-model/src/test/java/software/amazon/smithy/model/loader/smf/
smithy-model/src/test/resources/software/amazon/smithy/model/loader/smf/
```

#### Unit Tests

**LEB128Test.java**
- Encode/decode round-trip for boundary values: 0, 1, 127, 128, 16383,
  16384, 2^31-1 (max VarUInt).
- VarInt: 0, -1, 1, -64, 63, Long.MIN_VALUE, Long.MAX_VALUE.
- Reject encodings longer than 5 bytes (VarUInt) or 10 bytes (VarInt).
- Reject non-minimal encodings (leading zero bytes).

**SmfSharedSymbolsTest.java**
- Verify all entries are valid shape IDs or known strings.
- Verify no duplicates.
- Verify IDs are contiguous starting at 1.
- Verify reverse lookup is consistent with forward lookup.

**SmfConstantsTest.java**
- Verify shape type enum covers all `ShapeType` values.
- Verify no gaps or collisions in enum values.

#### Writer Tests

**SmfWriterTest.java**
- Write a minimal model (one simple shape) and verify header bytes manually.
- Write a model with each shape type and verify the shape type byte.
- Write a model with traits and verify trait encoding.
- Write a model with metadata and verify metadata section.
- Write with `stripped=true` and verify documentation traits are absent.
- Verify symbol table frequency ordering (most-used symbol gets lowest ID).
- Verify shared symbols are not duplicated in local table.
- Verify shape index offsets are correct (can seek to each shape).

#### Reader Tests

**SmfReaderTest.java**
- Read back every file written by `SmfWriterTest` and verify the model
  matches.
- Reject files with wrong magic number.
- Reject files with unsupported format version.
- Reject files with truncated data (mid-shape, mid-symbol-table).
- Reject files with invalid shape type bytes.
- Reject files with VarUInt > 5 bytes.
- Reject files with symbol ID 0 in a SymRef position.
- Reject files with symbol ID beyond table bounds.

#### Round-Trip Tests

**SmfRoundTripTest.java** (parameterized)

The most critical test suite. For every existing test model:

1. Load the model from JSON AST (or .smithy).
2. Write it to SMF via `SmfWriter`.
3. Read it back via `SmfReader`.
4. Serialize both models to JSON AST via `ModelSerializer`.
5. Assert the two JSON ASTs are equal (using `Node.assertEquals`).

This runs against:
- All files in `loader/valid/*.json` (~63 models).
- All files in `shapes/ast-serialization/cases/**/*.json` (~21 models).
- A set of purpose-built models covering edge cases (see below).

**SmfRoundTripRunnerTest.java** (runner pattern, like
`ValidSmithyModelLoaderRunnerTest`)

Discovers all `.json` model files in a test resource directory and
automatically round-trips each one. Adding a new test case is just adding
a `.json` file.

#### Edge Case Models

Purpose-built test resources in `loader/smf/cases/`:

```
empty-model.json                  // No shapes, no metadata
simple-shapes.json                // One of each simple shape type
structure-members.json            // Structure with ordered members
structure-no-members.json         // Empty structure
enum-values.json                  // Enum with @enumValue traits
int-enum-values.json              // IntEnum with integer enumValues
list-with-member-traits.json      // List member has traits
map-shape.json                    // Map with key/value traits
operation-full.json               // Operation with input, output, errors
operation-minimal.json            // Operation with no input/output/errors
resource-full.json                // Resource with all lifecycle ops
resource-minimal.json             // Resource with no lifecycle ops
service-full.json                 // Service with operations, resources, errors, renames
service-minimal.json              // Service with only version
mixins-structure.json             // Structure with mixins, members from mixins
mixins-operation.json             // Operation with mixins
annotation-traits.json            // Multiple annotation traits ({} value)
complex-trait-values.json         // Traits with nested objects, lists, strings
numeric-trait-values.json         // Integer, double, bigInteger, bigDecimal values
documentation-heavy.json          // Large documentation strings
metadata-various.json             // Metadata with all value types
shared-symbol-targets.json        // Shapes targeting prelude types (smithy.api#String etc.)
many-shapes.json                  // 500+ shapes to test symbol table scaling
unicode-strings.json              // Member names and docs with multi-byte UTF-8
null-trait-value.json             // Trait with explicit null value
apply-merged.json                 // Model where apply was already resolved
set-shape.json                    // Deprecated set shape type
```

#### Selective Loading Tests

**SmfSelectiveLoadTest.java**
- Load a model with a service + multiple operations.
- Selectively load 1 operation and verify only the transitive closure is
  present.
- Verify that shapes outside the closure are not in the resulting model.
- Verify that common error shapes referenced by multiple operations are
  included.
- Verify that prelude shapes are handled correctly (not expected in file
  or expected in file, depending on open question resolution).

#### Integration Tests

**SmfModelAssemblerTest.java**
- Load a `.smf` file via `Model.assembler().addImport(path)` and verify
  it produces a valid model.
- Load a `.smf` file alongside a `.smithy` file and verify they merge
  correctly.
- Load a `.smf` file from inside a JAR and verify discovery works.
- Verify that validation runs correctly on SMF-loaded models.

#### Performance Tests (not in CI, manual/benchmark)

**SmfBenchmark.java** (JMH or manual timing)
- Load the EC2 model from JSON AST, measure time.
- Load the same model from SMF, measure time.
- Compare allocation counts (via `-XX:+PrintGCDetails` or allocation
  profiler).
- Measure selective loading time for 1, 5, 20 operations from EC2.

#### Fuzz Tests (src/it)

Fuzz testing uses [Jazzer](https://github.com/CodeIntelligenceTesting/jazzer)
for coverage-guided fuzzing, following the same pattern proven in smithy-java's
CBOR codec (which found real bugs). These live under `smithy-model/src/it/`
and run separately from CI (long-running, non-deterministic).

**Directory structure:**
```
smithy-model/src/it/
├── java/software/amazon/smithy/model/loader/smf/
│   ├── SmfDeserializationFuzzTest.java
│   └── SmfRoundTripFuzzTest.java
└── resources/
    ├── dictionary/smf-fuzz.dict
    └── corpus/   (seeded with valid .smf files, grows over time)
```

**SmfDeserializationFuzzTest.java**

Feeds arbitrary byte arrays to `SmfReader` and asserts that it either:
- Produces a valid `Model`, or
- Throws a clean, expected exception (`SmfFormatException`,
  `IllegalArgumentException`, `IndexOutOfBoundsException`)

It MUST NOT:
- Throw `NullPointerException`
- Throw `StackOverflowError` (from deeply nested DynamicValues)
- Enter an infinite loop (timeout enforced)
- Produce a `Model` with corrupted data (shapes with null IDs, etc.)
...
public void fuzzReader(byte[] input) {
    assertTimeoutPreemptively(Duration.ofSeconds(5), () -> {
        ...
        } catch (SmfFormatException | IllegalArgumentException
...
Uses Jazzer's structured fuzzing to generate random but *valid* models
(random shape counts, random member counts, random trait values), writes
them to SMF, reads them back, and asserts equivalence. This catches
encoding/decoding asymmetries.

```java
@FuzzTest
public void fuzzRoundTrip(FuzzedDataProvider data) {
    Model model = generateRandomModel(data);
    byte[] encoded = SmfWriter.write(model);
    Model decoded = SmfReader.read(encoded);
    assertModelsEquivalent(model, decoded);
}
```

**smf-fuzz.dict**, Dictionary of interesting byte sequences:
...
# Common patterns: empty model (magic + version + smithy 2.0 + flags + symtab + index + shapes)
"\x53\x4D\x42\x59\x01\x02\x00\x00\x01\x01\x00\x00\x00"

# Annotation trait (empty object tag)
"\x07"
...
The initial corpus is generated by writing all existing test models to SMF:
- All `loader/valid/*.json` models
- All `shapes/ast-serialization/cases/**/*.json` models
- The edge case models from `loader/smf/cases/`

This gives Jazzer a starting point of valid files to mutate, dramatically
improving coverage compared to starting from random bytes.

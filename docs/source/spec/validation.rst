...
.. _HttpMethodSemantics:

HttpMethodSemantics
-------------------

Validates that :ref:`http <http-trait>` traits applied to :ref:`operation`
shapes use the most semantically appropriate HTTP method according to
:rfc:`7231`.

Rationale
    Using semantically appropriate HTTP methods makes it easier to consume
    a Smithy web service that uses an HTTP-based protocol using non-Smithy
    clients.

Default severity
    ``WARNING``

This validator emits validation events under the following circumstances:

1. The "behavior" property of the targeted operation is different than the
   expected behavior of the operation based on the uppercase value of the
   ``method`` property.

2. The uppercase :ref:`http-trait` ``method`` property value applied to
   a lifecycle operation has different semantics than the operation.

The following table defines the expected safety for each HTTP method checked
by this validator:

===========   ===========================   ==========================
HTTP method   Expected operation behavior   Requires idempotent trait?
===========   ===========================   ==========================
GET           read                          no
HEAD          read                          no
OPTIONS       read                          no
...
.. code-block:: smithy
    ...
    metadata validators = [{name: "HttpMethodSemantics"}]


.. _HttpResponseCodeSemantics:

HttpResponseCodeSemantics
-------------------------

Validates that :ref:`http-trait` ``code`` properties and the
``httpError`` property of an :ref:`error-trait` are semantically
appropriate for operations and errors.

Rationale
    Using semantically appropriate HTTP status codes makes it easier to consume
    a Smithy web service that uses an HTTP-based protocol using non-Smithy
    clients.
...
This validator emits a validation event under the following circumstances:

#. An operation has a non-2xx level ``code`` applied. The ``code`` of an
   operation is used to indicate a successful response.
#. A client error structure uses a ``code`` outside of the 4xx range.
#. A server error structure uses a ``code`` outside of the 5xx range.

Example:

.. code-block:: smithy

    metadata validators = [{name: "HttpResponseCodeSemantics"}]

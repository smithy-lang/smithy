...
.. list-table::
    ...
    * - message
      ...
      - A JMESPath expression that extracts a human-readable message from the
        ...
        resolve to a ``string`` or an array of ``string`` values. This is
        used to provide actionable information (for example, failure reasons)
        to the waiter caller. This property is only valid on acceptors with a
        ``failure`` state.

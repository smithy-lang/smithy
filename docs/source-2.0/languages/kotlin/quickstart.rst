...
This guide introduces `Smithy Kotlin <https://github.com/smithy-lang/smithy-kotlin>`_ with a simple working example of a
generated server and client.

For this example, imagine that you are the proud owner of a coffee shop.
Your API allows your customers to order some coffee from their Kotlin applications.
...
* Ensure you have JDK 8 or higher installed. Older versions are not supported by Smithy Kotlin.
...
Clone the `quickstart example template <https://github.com/smithy-lang/smithy-examples/tree/main/smithy-kotlin-examples/quickstart-kotlin>`_
using the Smithy CLI ``init`` command and change to the cloned directory:

.. code-block:: sh

    smithy init -t smithy-kotlin-quickstart && cd smithy-kotlin-quickstart
...
* ``server/``: Code generated Java server that implements stubbed operations code-generated from the service model.
  Smithy Kotlin doesn't `currently <https://github.com/smithy-lang/smithy-kotlin/issues/1011>`_ support server code generation.
  The server is being code generated using :doc:`Smithy Java </languages/java/index>`.
* ``gradle/``:  Gradle configuration directory that contains a `version catalog <https://docs.gradle.org/current/userguide/version_catalogs.html>`_.
...
The service has the ``@restJson1`` and ``@rpcv2Cbor`` protocol trait applied indicating that the service supports the :ref:`AWS restJson1 protocol <aws-restjson1-protocol>` and :ref:`rpcv2Cbor protocol <smithy-rpc-v2-cbor>`.
...
The command will code generated a Kotlin client and start the client application for the coffee shop service to:
...
.. code-block:: sh
    ...
    INFO: Creating coffee order
    INFO: Created order with ID: fea16a63-c331-497d-9551-cb566939da05
...
We'll add a new operation to our service that allows users to get the hours of our cafe.

The new operation, ``GetHours``, will be bound directly to our service shape, send the day of the week as input, and will return an output
...
.. code-block:: diff
    ...
    + /// Retrieve the coffee shop hours depending on the day.
    + @http(method: "GET", uri: "/hours/{day}")
    + @readonly
    + operation GetHours {
    +     input := {
    +         @httpLabel
    ...
    + /// An error indicating the day of the week could not be found
    + @httpError(404)
    + @error("client")
    + structure DayNotFound {
    +     message: String
    + }
...
.. code-block:: java
    :caption: server/src/main/java/io/smithy/kotlin/server/example/GetHours.java

    import io.smithy.kotlin.server.example.model.DayNotFound;
    import io.smithy.kotlin.server.example.model.GetHoursInput;
    import io.smithy.kotlin.server.example.model.GetHoursOutput;
    import io.smithy.kotlin.server.example.service.GetHoursOperation;
    ...
    final class GetHours implements GetHoursOperation {
        ...
        public GetHoursOutput getHours(GetHoursInput input, RequestContext context) {
            var outputBuilder = GetHoursOutput.builder();

            return switch (input.day().value()) {
                ...
                case "SATURDAY", "SUNDAY" ->
                    outputBuilder
                        .opensAt(10) // Opens at 10AM
                        .closesAt(14) // Closes at 2PM
                        .build();
                default ->
                    throw DayNotFound.builder()
                        .message("Day not found: " + input.day().value())
                        .build();
            };
        }
    }
...
.. code-block:: diff
    :caption: server/src/main/java/io/smithy/kotlin/server/example/CafeService.java
...
.. code-block:: sh
    ...
    curl localhost:8888/hours/MONDAY
...
.. code-block:: diff
    :caption: client/src/main/kotlin/io/smithy/kotlin/client/example/Main.kt
    ...
    +    val mondayHours = client.getHours {
    +        day = Day.Monday
    +    }

    +    logger.info("Monday hours: $mondayHours")
       ...
    }
...
.. code-block:: sh
    ...
    INFO: Monday hours: GetHoursResponse(closesAt=16,opensAt=9)

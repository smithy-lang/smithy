...
class CallCommandTest {
    ...
    @Test
    void closureCommandHintIsCopyable() {
        String hint = CallCommand.closureCommand("s3", ShapeId.from("com.amazonaws.s3#PutObject"));

        // Exact form the help and error hints surface: --no-docs for compactness, an unquoted shape id,
        // and the closure selector. Must be runnable as-is (no surrounding punctuation that would break a
        // copy-paste), so we assert the whole string.
        assertEquals(
                "smithy ast --name s3 --no-docs --selector '[id = com.amazonaws.s3#PutObject] :is(*, ~>)'",
                hint);
    }

...
public class ModifiedTraitTest {
    ...
    public void modifiedShapeNoTag() {
        ...
        assertThat(
            events.stream().filter(e -> e.getSourceLocation().getFilename().endsWith("a.smithy")).count(),
            equalTo(1L)
        );
        assertThat(events.stream().filter(e -> e.getSeverity() == Severity.NOTE).count(), equalTo(2L));
        assertThat(
            events.stream().filter(e -> e.getSourceLocation().getFilename().endsWith("b.smithy")).count(),
            equalTo(2L)
        );

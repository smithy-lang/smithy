...
public class RemovedOperationErrorTest {
    ...
    public void detectsRemovedErrors() {
        ...
        assertThat(
            ...
            startsWith(
                "[WARNING] foo.baz#Operation: The `foo.baz#E1` error was removed " +
                    "from the `foo.baz#Operation` operation. | RemovedOperationError"
            )
        );
        assertThat(
            TestHelper.findEvents(events, "RemovedOperationError").get(1).toString(),
            startsWith(
                "[WARNING] foo.baz#Operation: The `foo.baz#E2` error was removed " +
                    "from the `foo.baz#Operation` operation. | RemovedOperationError"
            )
        );
    }

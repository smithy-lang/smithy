...
public class AddedRequiredMemberTest {
    ...
    public void addingRequiredTraitWithoutDefaultIsAnError() {
        ...
        assertThat(
            TestHelper.findEvents(result.getDiffEvents(), "AddedRequiredMember").get(0).getShapeId().get().toString(),
            equalTo("smithy.example#A$foo")
        );
        assertThat(
            TestHelper.findEvents(result.getDiffEvents(), "AddedRequiredMember").get(0).getMessage(),
            equalTo(
        ...
        assertThat(
            TestHelper.findEvents(result.getDiffEvents(), "AddedRequiredMember").get(0).getSourceLocation(),
            equalTo(source)
        );
    }

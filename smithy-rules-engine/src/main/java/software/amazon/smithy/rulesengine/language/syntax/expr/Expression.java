...
public abstract class Expression extends MandatorySourceLocation implements TypeCheck, ToNode {
    ...
    public static Expression fromNode(Node node) {
        if (node.asObjectNode().isPresent()) {
            ...
            if (ref.isPresent()) {
                return reference(Identifier.of(ref.get().expectStringNode("ref must be a string")), ref.get());
            }
        ...
        return context("while parsing `" + shortForm + "` within a template", context, () -> {
            if (shortForm.contains("#")) {
                ...
                return GetAttr.builder()
                        ...
                        .target(reference(Identifier.of(base), context))
                        .path(pattern).build();
            } else {
                return Expression.reference(Identifier.of(shortForm), context);
            }
        });
    }
    ...
    public static Literal literal(StringNode node) {
        return Literal.string(new Template(node));
    }

...
public final class AuthorizersTraitValidator extends AbstractValidator {
    ...
    private List<ValidationEvent> validate(Model model, ServiceShape service) {
        ...
        Optional<ValidationEvent> authSchemaValidation = validateAuthSchema(authorizers, model, service);
        authSchemaValidation.ifPresent(validationEvents::add);

        Optional<ValidationEvent> enableSimpleResponsesValidation = validateEnableSimpleResponsesConfig(
            authorizers,
            service
        );

...
public class OpenApiConfig extends JsonSchemaConfig {
    ...
    static {
        ...
        DEPRECATED_PROPERTY_RENAMES.put(
            "apigateway.disableCloudFormationSubstitution",
    ...
    private static ObjectNode fixDeprecatedKeys(ObjectNode node) {
        ...
        for (Map.Entry<String, Node> entry : mapped.getStringMap().entrySet()) {
            if (DEPRECATED_PROPERTY_RENAMES.containsKey(entry.getKey())) {
                ...
                LOGGER.warning(
                    "Deprecated `openapi` configuration setting found: " + entry.getKey()
                        + ". Use " + rename + " instead"
                );
                ...
                throw new OpenApiException(
                    "Unsupported `openapi` configuration setting found: " + entry.getKey()

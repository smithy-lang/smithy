...
if (project.hasProperty("awsModelsTests")) {
    ...
    tasks.named<Test>("integ") {
        ...
        val modelFilter = findProperty("smfAwsModelFilter")?.toString()
            ?: System.getProperty("smfAwsModelFilter")
        if (modelFilter != null) {
            systemProperty("smfAwsModelFilter", modelFilter)
        }
        val modelFilterMode = findProperty("smfAwsModelFilterMode")?.toString()
            ?: System.getProperty("smfAwsModelFilterMode")
        if (modelFilterMode != null) {
            systemProperty("smfAwsModelFilterMode", modelFilterMode)
        }
        maxHeapSize = "4g"
    }
}

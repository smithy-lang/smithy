...
final class SmfConstants {
    ...
    static final int FLAG_HAS_TRAIT_VALUE_OFFSETS = 0x04;
    static final int FLAG_SPLIT_COLD_TRAITS = 0x08;
    ...
    // Object whose keys are symbol-table references (SymRef) instead of inline
    // strings. Used when every key is already interned in the symbol table
    // (the common case for trait-value objects, whose keys are member names).
    // Falls back to VALUE_OBJECT (0x07) when any key is not interned.
    static final byte VALUE_OBJECT_SYMREF_KEYS = 0x0B;
    // String value encoded as a symbol-table reference instead of inline UTF-8.
    // Used when the string value is already interned (e.g. enumValue,
    // ec2QueryName values that match a member name). Falls back to
    // VALUE_STRING (0x05) otherwise.
    static final byte VALUE_STRING_SYMREF = 0x0C;

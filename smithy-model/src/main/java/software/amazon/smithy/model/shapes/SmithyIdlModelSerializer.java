...
public final class SmithyIdlModelSerializer {
    ...
    private static final class ShapeSerializer extends ShapeVisitor.Default<Void> {
        ...
        public Void operationShape(OperationShape shape) {
            ...
            List<MemberShape> mixinMembers = new ArrayList<>();
            shape.getInput().ifPresent(shapeId -> {
                mixinMembers.addAll(writeInlineableProperty("input", shapeId, shape.getId().getName() + "Input"));
            });
            shape.getOutput().ifPresent(shapeId -> {
                mixinMembers.addAll(writeInlineableProperty("output", shapeId, shape.getId().getName() + "Output"));
            });
            codeWriter.writeOptionalIdList("errors", shape.getErrors());
            codeWriter.closeBlock("}");
            codeWriter.write("");

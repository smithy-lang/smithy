...
public final class ResourceOperationInputOutputValidator extends AbstractValidator {
    ...
    private List<ValidationEvent> validateResource(Model model, ResourceShape resource) {
        ...
        processLifecycleOperationProperties(
            model,
            resource,
            "put",
            resource.getPut(),
        ...
        for (String propertyNotInLifecycleOp : definedProperties) {
            events.add(
                error(
                    resource,
                    String.format(
                        "Resource property `%s` is not used in the input or output"
                            + " of create or an instance operation.",
    ...
    private void validateOperationOutput(
        ...
        Set<String> identifierMembers = new HashSet<>(
        ...
        if (properties.containsKey(propertyName)) {
            ...
            events.add(
                error(
                    member,
                    String.format(
                        "Member `%s` does not target a property or identifier"
                            + " for resource `%s`",

...
public class SpecIntegTest {
    ...
    public void attributeExistence_MatchesShapesWithTrait() {
        ...
        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|deprecated]"),
            containsInAnyOrder("smithy.example#DeprecatedString")
        );

        // Empty tags traits still exist.
        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|tags]"),
            containsInAnyOrder("smithy.example#MyString2")
        );

        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|enum]"),
            containsInAnyOrder(
    ...
    public void attributeExistence_MatchesShapesWithProjection() {
        ...
        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|enum|(values)]"),
            containsInAnyOrder(
                "smithy.example#MyString3",
                "smithy.example#MyString4",
                "smithy.example#MyString5"
        ...
        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|enum|(values)|tags]"),
            containsInAnyOrder("smithy.example#MyString5")
        );

        // An empty projection does not exist.
        assertThat(
            SelectorTest.exampleIds(attributeExistenceModel, "[trait|enum|(values)|tags|(values)]"),
            containsInAnyOrder("smithy.example#MyString5")
        );
    }
    ...
    public void allowedTags_illustratesWhyProjectionComparatorsExist() {
        Set<String> noneMatch = SelectorTest.ids(
            allowedTagsModel,
            "service\n"
                + "[trait|smithy.example#allowedTags]\n"
                + "$service(*)\n"
        ...
        Set<String> notWhatWeWanted = SelectorTest.ids(
            allowedTagsModel,
            "service\n"
                + "[trait|smithy.example#allowedTags]\n"
                + "$service(*)\n"

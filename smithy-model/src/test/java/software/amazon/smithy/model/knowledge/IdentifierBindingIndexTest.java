...
public class IdentifierBindingIndexTest {
    ...
    public void returnsEmptyMapForUnknownBindings() {
        ...
        assertThat(
            index.getOperationBindingType(ShapeId.from("ns.foo#A"), ShapeId.from("ns.foo#B")),
            equalTo(IdentifierBindingIndex.BindingType.NONE)
        );
        assertThat(
            index.getOperationInputBindings(ShapeId.from("ns.foo#A"), ShapeId.from("ns.foo#B")),
    ...
    public void findsImplicitInstanceBindings() {
        ...
        assertThat(
            index.getOperationBindingType(resource.getId(), operation.getId()),
    ...
    public void findsCollectionBindings() {
        ...
        assertThat(
            index.getOperationInputBindings(resource.getId(), operation.getId()),
            equalTo(Collections.emptyMap())
        );
        assertThat(
            index.getOperationBindingType(resource.getId(), listOperation.getId()),
            equalTo(IdentifierBindingIndex.BindingType.COLLECTION)
        );
        assertThat(
            index.getOperationInputBindings(
                resource.getId(),
                listOperation.getId()
            ),
            equalTo(Collections.emptyMap())

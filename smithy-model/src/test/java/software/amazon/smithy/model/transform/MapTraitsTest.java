...
public class MapTraitsTest {
    ...
    public void appliesAllMappersToShapes() {
        ...
        Model model = Model.builder().addShape(shape).build();
        ModelTransformer transformer = ModelTransformer.create();
        Model result = transformer.mapTraits(
            model,
            Arrays.asList(
                (s, t) -> {
        ...
        assertThat(
            result.expectShape(shapeId).getTrait(DeprecatedTrait.class).get().getMessage().get(),
            Matchers.equalTo("baz")
        );
        assertThat(
            result.expectShape(shapeId).getTrait(DocumentationTrait.class).get().getValue(),

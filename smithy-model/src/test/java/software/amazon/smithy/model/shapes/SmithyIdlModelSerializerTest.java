...
public class SmithyIdlModelSerializerTest {
    private static final URL TEST_FILE_URL = Objects.requireNonNull(
        SmithyIdlModelSerializer.class.getResource("idl-serialization/cases")
    );
    ...
    public void transientTraitsAreNotSerialized() {
        ...
        assertThat(
            results.get(Paths.get("com.foo.smithy")),
    ...
    public void serializesRootLevelDefaults() {
        ...
        assertThat(
            model.expectShape(ShapeId.from("smithy.example#PrimitiveBool")).hasTrait(DefaultTrait.ID),
            is(true)
        );
        assertThat(
            model2.expectShape(ShapeId.from("smithy.example#PrimitiveBool")).hasTrait(DefaultTrait.ID),
            is(true)
        );

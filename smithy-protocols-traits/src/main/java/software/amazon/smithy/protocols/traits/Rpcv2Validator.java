...
/**
 * Ensures that every entry in {@code eventStreamHttp} also appears in the {@code http} property of
 * a protocol trait.
 */
@SmithyInternalApi
public final class Rpcv2Validator extends AbstractValidator {
            ...
            if (protocol instanceof Rpcv2Trait) {
                ...
                if (!formats.isEmpty()) {
                    events.add(error(service, protocol, String.format(        
                            ...
                            + "are not supported: %s", protocol.toShapeId(), formats)));;
                }
            }
        }
